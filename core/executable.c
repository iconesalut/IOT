                    ...
                    for(j = count;j > 0;j--)
                    {
                        ...
                        if(f != 0)
                            if(f->optimizable)
                            {
                                ...
                                instruction_ptr->instruction = insn[j];
                                instruction_ptr->label = 0;
                                instruction_ptr->function = f;
                                instruction_ptr->next = f->firstInstruction;
...
void loadExecutable(Executable* exe, const char* name)
{
    ...
    extractFunction(exe);
    printf("-------------disassembly the executable-----------------\n");
    disassembly(exe);
}
...
void unloadExecutable(Executable* exe)
{
    ...
    for(i = 0;i < exe->numberFunction;i++)
        ...
        {
            Instruction* instruction = exe->functions[i].firstInstruction;
